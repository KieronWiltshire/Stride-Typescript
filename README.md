...
> - Run the `npm run start` command in order to start the server. The start script should use [forever][1] to help ensure that your content is continuously being served.
> - Run the `npm run start:dev` command in order to start the server in development mode. The develop script uses [nodemon][6] to restart based on changes made within the `src` directory (_This command should not be used in a production environment_).

## Available Scripts

- `start` - starts the server,
- `start:dev` - starts a server directly from the source that reloads on changes,
...
- `mogront` - creates and manages database migrations and seeders. ([Docs][10])
...
[10]: https://github.com/KieronWiltshire/node-mogront/blob/master/readme.md#using-the-cli